...
|---|---|---|---|---|---|
| 1 | `data_export.json`에 운영 데이터 + 고객명 노출 (공개 GitHub 저장소) | 보안 | 높음 (이미 발생) | 높음 | `data_export.json` |
...
### 3.1 ★ data_export.json 운영 데이터 노출 (심각)
...
- **상세**:
  ...
  - 단, `data_export.json` (언더스코어 없음)은 패턴 불일치로 추적됨
...
### 즉시 조치 필요 (마이그레이션 전)
1. **data_export.json Git 히스토리 제거** + .gitignore 수정 (보안 3.1)
